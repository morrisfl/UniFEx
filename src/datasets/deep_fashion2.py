...
class DeepFashion2(Dataset):
    def __init__(self, root, min_samples=3, max_samples=None, num_cls=None, transform=None, augmentation=None,
        ...
        """
        self.root = os.path.join(root, "deepfashion2")
        ...
        self.df["label"] = self.df["label"].map(ids_to_label)
    ...
    def _sampling(self):
        ...
        ids = sorted(list(self.df["label"].unique()))
...
if __name__ == "__main__":
    ...
    dataset = DeepFashion2(data_root, transform=img_transform)
